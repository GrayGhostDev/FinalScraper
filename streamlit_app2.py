...
def main():
    ...
    if st.session_state['scraping_state'] == 'completed' and st.session_state['results']:
        ...
        if len(all_data) > 0:
            ...
            for i, data in enumerate(all_data, start=1):
                ...
                st.write("Select data cleaning options below:")

                # Handle missing values
                missing_value_options = st.multiselect(
                    "Choose columns to fill missing values:",
                    options=df.columns.tolist(),
                    key=f"missing_values_{i}"
                )
                fill_value = st.text_input("Fill missing values with:", "", key=f"fill_value_{i}")

                # Remove duplicates
                remove_duplicates = st.checkbox(
                    "Remove duplicate rows",
                    value=True,
                    key=f"remove_duplicates_{i}"
                )

                # Apply data cleaning
                if st.button("Apply Data Cleaning", key=f"apply_cleaning_{i}"):
                ...
                st.write("Select data analysis options below:")
                ...
                selected_columns = st.multiselect(
                    ...
                    key=f"analysis_columns_{i}"
                )

                # If columns selected, show statistics
                if selected_columns:
                    ...
                    st.info("Select at least one numeric column to analyze.")
        ...
        if st.sidebar.button("Clear Results", key="clear_results_button"):
            # Reset session state
            st.session_state['scraping_state'] = 'idle'
    ...
    if st.session_state['scraping_state'] == 'idle':
        st.write("Awaiting input to start scraping.")
